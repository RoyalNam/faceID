...
class FaceDetector:
    ...
    def prepare(self, ctx_id, **kwargs):
        ...
        if input_size is not None:
            if self.input_size is not None:
                print("warning: det_size is already set in model detection, ignore")
            else:
    ...
    def forward(self, img, thresh):
        ...
        blob = cv2.dnn.blobFromImage(
            img, 1.0 / 128, input_size, (127.5, 127.5, 127.5), swapRB=False
        )
    ...
    def detect(
        ...
        if kpss is not None:
            ...
            landmarks = []  # Or you can return an empty list or handle it differently
        #
        # landmarks = np.int32(kpss)

        return bboxes, landmarks

    def find_pose(self, points):
        """
        Parameters
        ----------
